 ...
 * WX keycode.
 */
int wxCharCodeXToWX(int keySym);
int wxUnicodeCharXToWX(int keySym);
...
int wxUnicodeCharXToWX(int keySym)
{
    ...
    while ( max >= min ) {
        mid = (min + max) / 2;
        if ( keySymTab[mid].keySym < (unsigned int)keySym )
            min = mid + 1;
        else if ( keySymTab[mid].keySym > (unsigned int)keySym )
            max = mid - 1;
        else {
            /* found it */
...
#else // defined(__WXGTK__)
    if (IsItWayland()) {
        wxLogError(wxT("Wayland does not allow to globally query keypresses for security reason. \
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        if (currentState[i] != previousState[i]) {
            ...
            for (int pos = 0; pos < 8; ++pos) {
                ...
                int xKeySym = wxUnicodeCharXToWX(XkbKeycodeToKeysym(x11display, rawKeycode, 0, 1)); // same as WX keycode wxEVT_KEY_DOWN
                // Key was pressed
